...
(defun khst/cmd-remove-deprecated (json args &key reply at &allow-other-keys)
                ...
                (progn
                  (reply-to *napcat-websocket-client*
                            json (make-message `(:image ,*khst-are-you-sure*
                                                 :sub-type 1)))
                  (return-from khst/cmd-remove-deprecated))
                (setf res "格式错误"))
...
(defun khst/cmd-khst-deprecated (json args &key &allow-other-keys)
  (let* ((msg-type (@ json "message_type"))
