...
(defun main ()
  ...
  (setq *onebot-websocket-client*
        (wsd:make-client "ws://localhost:3001"))
  (wsd:start-connection *onebot-websocket-client*)
  (wsd:on :message *onebot-websocket-client*
          (lambda (message)
...
(defun sender/send-message (message)
  (format t "~A~%" message)
  (wsd:send-text *onebot-websocket-client* message))
...
(defun nitory/dice-expr-leading-p (leading)
  (or (nitory/dice-expr-legal-leading-p leading)
      (nitory/dice-expr-legal-operator-p leading)
      (nitory/dice-term-legal-operator-p leading)
      (char= #\) leading)
      (char= #\# leading)))
...
(defun nitory/dice-expr-op-p (op op-category)
  (case op-category
    (:expr (or (eq op :+)
               (eq op :-)))
    (:term (or (eq op :*)
...
(defun nitory/rotate-fix- (tree op-category)
  (if (and (nitory/dice-expr-op-p (car tree) op-category)
           (nitory/dice-expr-op-p (caaddr tree) op-category))
...
(defun nitory/parse-dice-cell (cell)
  (let ((leading (ignore-errors (char cell 0))))
    (assert (and (> (length cell) 0)
                 (nitory/dice-expr-legal-leading-p leading)))
    (if (char= #\( leading)
...
(defun nitory/parse-dice-term- (term)
  (let* ((rp1 (nitory/parse-dice-cell term))
    ...
    (if (or (>= pos1 (length term))
            (nitory/dice-expr-legal-operator-p op)
            (char= #\) op))
        rp1
        (progn
          (assert (nitory/dice-term-legal-operator-p op))
...
(defun nitory/parse-dice-expr- (expr)
  (let* ((rp1 (nitory/parse-dice-term expr))
    ...
    (if (or (>= pos1 (length expr))
        ...
        rp1
        (progn
          (assert (nitory/dice-expr-legal-operator-p op))
...
(defun nitory/join (str list)
  (declare (inline nitory/join))
  (reduce (lambda (x y)
            (nitory/concat x str y)) list))

(defun nitory/concat (&rest strings)
  (declare (inline nitory/concat))
...
(defun nitory/roll-dice (rest &optional sender)
  (let ((leading (if (= (length rest) 0)
    ...
    (let ((expr (handler-case (nitory/parse-dice-full-expr
                               (if space
                                   (subseq rest 0 space)
                                   (if (nitory/dice-expr-leading-p leading)
                                       rest
                                       "")))
                  (error () (return-from nitory/roll-dice "掷骰失败"))))
          (comment (string-trim
                    " "
                    (if space
                        (subseq rest (+ 1 space))
